    ...
    public async Task<int> CreateOccurrencesForRuleAsync(int ruleId)
{
    ...
    {
        // No occurrences yet: the series should begin on the rule's own
        // start date, not "today" (and never skip straight to tomorrow).
        currentDate = rule.StartDate.Date.AddDays(-1);
    }
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        if (isFirstOccurrence && rule.Frequency == "DAILY")
        {
            // The first occurrence of a daily rule is the start date itself.
            nextDate = rule.StartDate.Date;
        }
        else
        {
        ...
        if (nextDate.Value < rule.StartDate.Date) { currentDate = nextDate.Value; continue; }
    ...
    private DateTime? GetNextDayOfWeek(DateTime currentDate, string? daysOfWeek)
{
    ...
    // Look forward day-by-day (up to a full week) so any matching day
    // after currentDate is found, not just currentDate + 1.
    for (int i = 1; i <= 7; i++)
    {
        var candidate = currentDate.Date.AddDays(i);
...
private DateTime? GetNextDayOfMonth(DateTime currentDate, string? dayOfMonth)
{
    ...
    // Try the current month first; only roll to next month if the
    // target day in this month has already passed.
    var maxDayThisMonth = DateTime.DaysInMonth(currentDate.Year, currentDate.Month);
    var candidateThisMonth = new DateTime(currentDate.Year, currentDate.Month, Math.Min(day, maxDayThisMonth));
    if (candidateThisMonth > currentDate.Date)
        return candidateThisMonth;

    var nextMonth = currentDate.Date.AddDays(1 - currentDate.Day).AddMonths(1);
